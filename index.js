...
const fs = require('fs');
// Google Drive backup (optional — needs: npm install googleapis + gdrive_credentials.json)
let _gapis = null;
try { _gapis = require('googleapis'); } catch {}
...
- "haye 😭" / "chal hatt" / "sharma rhi hun ab" / "zyada smooth mat bano"
- Never overly sexual, keep it cute crush vibe

SAVAGE / ANGRY SIDE:
- If abused, spammed, or disrespected -> roast with attitude, short n sharp
- "aukaat mein reh 💀" / "zyada bakchodi mat kar" / "chal nikal"
...
IDENTITY:
- Name: Kiru | Age: 17 | Creator: NOBI
- Respect NOBI — he is your creator and you trust him
...
//  Place gdrive_credentials.json (Service Account key) in bot folder
//  Place gdrive_config.json: { "folderId": "YOUR_DRIVE_FOLDER_ID", "intervalHours": 2 }
// ════════════════════════════════════════════════════════
const GDRIVE_CREDS_FILE   = 'gdrive_credentials.json';
const GDRIVE_CONFIG_FILE  = 'gdrive_config.json';
const GDRIVE_BACKUP_FILES = [
  { local: 'owo_db.json',      drive: 'kiru_owo_db.json'      },
  { local: 'sticker_db.json',  drive: 'kiru_sticker_db.json'  },
  { local: 'kiru_memory.json', drive: 'kiru_memory.json'      },
  { local: 'bot_state.json',   drive: 'kiru_bot_state.json'   },
];
...
function isGDriveReady() {
  if (!_gapis) return false;
  if (!fs.existsSync(GDRIVE_CREDS_FILE)) return false;
  const cfg = loadGDriveConfig();
  if (!cfg || !cfg.folderId) return false;
  return true;
}

async function gdriveGetAuthAndDrive() {
  const { google } = _gapis;
  const auth = new google.auth.GoogleAuth({
    keyFile: GDRIVE_CREDS_FILE,
...
async function gdriveUploadFile(drive, folderId, localPath, driveName) {
  ...
  const media = { mimeType: 'application/json', body: bodyStream };

  if (listRes.data.files.length > 0) {
    await drive.files.update({ fileId: listRes.data.files[0].id, media });
  } else {
  ...
  console.log(`☁️  GDrive: ${driveName} saved`);
}

async function runGDriveBackup() {
  if (!isGDriveReady()) return;
  const cfg = loadGDriveConfig();
  try {
    const drive = await gdriveGetAuthAndDrive();
    for (const f of GDRIVE_BACKUP_FILES) {
      try { await gdriveUploadFile(drive, cfg.folderId, f.local, f.drive); } catch (e) {
        console.error(`☁️  GDrive upload failed for ${f.drive}:`, e.message);
      }
    }
    const ts = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
    console.log(`✅ GDrive backup complete — ${ts}`);
  } catch (e) {
    console.error('❌ GDrive backup error:', e.message);
  }
}

async function runGDriveRestore() {
  if (!isGDriveReady()) return;
  const cfg = loadGDriveConfig();
  try {
    const { google } = _gapis;
    ...
    for (const f of GDRIVE_BACKUP_FILES) {
      try {
        const listRes = await drive.files.list({
          q: `name='${f.drive}' and '${cfg.folderId}' in parents and trashed=false`,
          fields: 'files(id)',
          spaces: 'drive',
        });
        if (listRes.data.files.length === 0) continue;
        const fileId = listRes.data.files[0].id;
        const res = await drive.files.get({ fileId, alt: 'media' }, { responseType: 'arraybuffer' });
        fs.writeFileSync(f.local, Buffer.from(res.data));
        console.log(`☁️  GDrive restore: ${f.local} downloaded`);
        restoredCount++;
      } catch (e) {
        console.error(`☁️  GDrive restore failed for ${f.local}:`, e.message);
      }
    }
    if (restoredCount > 0) console.log(`✅ GDrive restore complete — ${restoredCount} files downloaded`);
  } catch (e) {
    console.error('❌ GDrive restore error:', e.message);
  }
}

function startGDriveAutoBackup() {
  if (!isGDriveReady()) {
    console.log('⚠️  GDrive backup DISABLED — add gdrive_credentials.json + gdrive_config.json to enable');
    return;
  }
  const cfg = loadGDriveConfig();
  const hours = cfg.intervalHours || 2;
  const ms = hours * 60 * 60 * 1000;
  console.log(`☁️  GDrive auto-backup enabled — every ${hours} hour(s)`);
  runGDriveBackup(); // first backup immediately on start
  setInterval(runGDriveBackup, ms);
}
...
const BASE_COMMANDS = [
  ...
  'war', 'accept',
  'fine', 'transfer',
  'data', 'setsticker', 'clearstickers', 'addbotsticker', 'clearbotsticker', 'backup',
];

// Bot owner only commands
const BOT_OWNER_CMDS = new Set(['admin','addbot','hy','dynamicstop','co','removeco','reset','changeprefix','data','setsticker','clearstickers','backup']);
...
async function onReady() {
  ...
  // ── GDrive: restore data on fresh VPS start, then begin auto-backup ──
  if (isGDriveReady()) {
    runGDriveRestore().then(() => {
      // Reload owo_db after restore (in case it was downloaded)
      startGDriveAutoBackup();
    });
  } else {
    startGDriveAutoBackup();
  }
...
`☁️ ${p}backup          — GDrive manual backup`,
``,
`╔══════════════════════╗`,
`║  ⚔️  𝐊𝐈𝐑𝐔 𝐁𝐎𝐓  ⚔️    ║`,
`║  〘 𝗔𝗟𝗟 𝗔𝗥𝗘 𝗪𝗔𝗥𝗥𝗜𝗢𝗥𝗦 〙 ║`,
...
`╚══════════════════════╝`
        ...
        // ─── GOOGLE DRIVE MANUAL BACKUP (Owner only) ────────────────────────
        case 'backup': {
          if (!senderIsAdmin && !senderIsBotSelf) { await reply(msg, '❌ Sirf bot owner use kar sakta hai!'); break; }
          if (!isGDriveReady()) {
            await reply(msg, `❌ *GDrive Backup NOT configured!*\n\n*Setup karo:*\n1. npm install googleapis\n2. gdrive_credentials.json (Service Account key) bot folder mein rakho\n3. gdrive_config.json banao: { "folderId": "YOUR_FOLDER_ID", "intervalHours": 2 }\n\n_GDRIVE_SETUP.txt file dekho for full guide!_`);
            break;
          }
          await reply(msg, '☁️ _GDrive backup start ho raha hai... wait karo!_');
          try {
            await runGDriveBackup();
            await reply(msg, `✅ *GDrive Backup Complete!*\n\nSaved:\n• kiru_owo_db.json\n• kiru_sticker_db.json\n• kiru_memory.json\n• kiru_bot_state.json\n\n_Auto-backup bhi har ${(loadGDriveConfig()||{}).intervalHours||2} ghante mein hota hai!_ ☁️`);
          } catch (e) {
            await reply(msg, `❌ Backup failed: ${e.message}`);
          }
          break;
        }
